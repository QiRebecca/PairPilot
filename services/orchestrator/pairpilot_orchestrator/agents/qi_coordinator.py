...
def build_qi_coordinator_agent(
    ...
    return Agent(
        ...
        instruction=(
            ...
            "concurrency. If that tool reports a transient contact_error, retry "
            "only the affected intent once when useful, or terminate safely; do "
            "not discard successful peer evidence from the same batch. The candidate "
