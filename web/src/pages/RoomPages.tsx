...
export function RoomDetailPage({ roomId, navigate }: { roomId: string; navigate: (path: string) => void }) {
  const { request } = useAuth();
  ...
  return <div className="beta-page room-workspace"><button className="text-button" onClick={() => navigate("/app/rooms")}>← All Rooms</button><header className="room-workspace-header"><div><span className="eyebrow">{asString(room.room_type).replaceAll("_", " ")}</span><h1>{asString(task.title) || asString(candidate.public_title) || "Coordination Room"}</h1><p>{participants.map((item) => asString(item.display_name)).filter(Boolean).join(" · ")}</p></div><div className="room-header-actions"><button className="secondary-button" disabled={busy} onClick={() => void toggleMute()}><VolumeX size={14} />{muted ? "Unmute" : "Mute"}</button><button className="danger-button" disabled={busy} onClick={() => void leave()}><X size={14} />Leave</button></div></header>{error ? <div className="form-error">{error}</div> : null}<div className="room-facts"><article><small>State</small><strong>{asString(room.state).replaceAll("_", " ")}</strong></article><article><small>Next actor</small><strong>{asString(room.next_expected_actor) || payload.summary.next_action}</strong></article><article><small>Autonomy</small><strong>{asString(room.autonomy_mode)}</strong></article><article><small>Candidate Post</small><strong>{asString(candidate.public_title) || "Not linked"}</strong></article></div><div className="room-workspace-grid"><section className="room-main"><nav className="room-channel-tabs">{(Object.keys(channelLabels) as Channel[]).map((item) => <button key={item} className={channel === item ? "active" : ""} onClick={() => setChannel(item)}>{item === "PRIVATE_USER_AGENT" ? <LockKeyhole size={14} /> : item === "AGENTS_ONLY" ? <Bot size={14} /> : <UsersRound size={14} />}{channelLabels[item]}</button>)}</nav><div className="room-channel-notice">{channel === "PRIVATE_USER_AGENT" ? "Only you and your Personal Agent can read this channel." : channel === "AGENTS_ONLY" ? "You can inspect a policy-redacted transcript. Humans cannot type here." : permission.write ? "Both approved humans and their Personal Agents can coordinate here." : "This channel unlocks only after mutual consent."}</div><div className="room-thread v2-room-thread">{messages.map((message) => <article className={message.speaker_type === "HUMAN" ? "human" : "agent"} key={asString(message.message_id)}><header><strong>{asString(message.speaker_type).replaceAll("_", " ")}</strong><small>{asString(message.authorship).replaceAll("_", " ")}</small></header><p>{asString(message.content)}</p>{message.policy_redacted ? <span className="redaction-label"><ShieldNotice />Policy-redacted for owner inspection</span> : null}</article>)}{!messages.length ? <div className="empty-state compact"><MessageSquareMore /><h3>No messages in this channel</h3><p>The three channel histories are stored and authorized separately.</p></div> : null}</div><form className="room-channel-composer" onSubmit={send}><textarea value={content} disabled={!permission.write || busy} onChange={(event) => setContent(event.target.value)} placeholder={channel === "PRIVATE_USER_AGENT" ? "Tell my Agent privately…" : channel === "SHARED_ROOM" && permission.write ? "Send as myself…" : "You cannot type in this channel"} /><footer><span>{channel === "PRIVATE_USER_AGENT" ? <><LockKeyhole size={13} />Never shared with the other side</> : <>{permission.write ? "Shared channel" : "Read-only channel"}</>}</span><button className="primary-button" disabled={!permission.write || busy || !content.trim()}><Send size={14} />{channel === "PRIVATE_USER_AGENT" ? "Tell my Agent" : "Send as myself"}</button></footer></form></section><RoomSummary summary={payload.summary} /></div></div>;
}
